 ...
 * @since 2.0.0
 */
class AffiliateWP_Manager_Recurring extends \Affiliate_WP_Recurring_Base {
	 ...
	 * @param   Referral $referral Referral parameters.
	 * @access  public
	 * @since   1.0
	 */
	...
	public function set_referral($data, $payment) {
		...
		if (!$referral || is_wp_error($referral)) {
			...
			if (!$payment_referral || is_wp_error($payment_referral)) {
				// Create a new referral for this payment

				/* translators: hash id of the customer */
				$desc = sprintf(__('New payment received from customer %s', 'wbi-wp-ultimo-affiliatewp'), $customer->get_hash());

			    $this->create_referral($referral, $affiliate_id, false, $desc, $membership, $payment);
				...
				if ($new_status !== $payment_referral->status && $payment_referral->status !== 'paid') {
					...
					if ($success) {
						...
						affiliate_wp()->utils->log(sprintf(__('Referral status of %1$s changed to "%2$s".', 'wp-ultimo-affiliatewp'), $payment_referral->referral_id, $new_status));
	...
	protected function create_referral($referral, $affiliate_id, $visit, $description, $membership, $payment) {
		...
		$referral_amount = $this->calc_referral_amount($amount, $reference, $referral ? $referral->referral_id : null, '', $affiliate_id);

		$referral_amount = (string) apply_filters('affwp_recurring_calc_referral_amount', $referral_amount, $affiliate_id, $amount); // phpcs:ignore
		...
		if (!$referral) {
			...
			$referral_id = affiliate_wp()->referrals->add( apply_filters( 'affwp_insert_pending_recurring_referral', $args, $this ) ); //phpcs:ignore
...
}  // end class AffiliateWP_Manager_Recurring;
