...
namespace WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Yoast\PHPUnitPolyfills\Polyfills;
...
use ReflectionType;
use TypeError;
use WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Yoast\PHPUnitPolyfills\Exceptions\InvalidComparisonMethodException;
/**
...
trait AssertObjectEquals
{
    ...
    public static function assertObjectEquals($expected, $actual, $method = 'equals', $message = '')
    {
        ...
        if (\is_object($expected) === \false) {
            throw new \TypeError(\sprintf('Argument 1 passed to assertObjectEquals() must be an object, %s given', \gettype($expected)));
        }
        if (\is_object($actual) === \false) {
            throw new \TypeError(\sprintf('Argument 2 passed to assertObjectEquals() must be an object, %s given', \gettype($actual)));
        }
        if (\is_scalar($method) === \false) {
            throw new \TypeError(\sprintf('Argument 3 passed to assertObjectEquals() must be of the type string, %s given', \gettype($method)));
        } else {
        ...
        if ($reflObject->hasMethod($method) === \false) {
            throw new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Yoast\PHPUnitPolyfills\Exceptions\InvalidComparisonMethodException(\sprintf('Comparison method %s::%s() does not exist.', \get_class($actual), $method));
        }
         ...
         */
        if ($reflMethod->getNumberOfParameters() !== 1 || $reflMethod->getNumberOfRequiredParameters() !== 1) {
            throw new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Yoast\PHPUnitPolyfills\Exceptions\InvalidComparisonMethodException(\sprintf('Comparison method %s::%s() does not declare exactly one parameter.', \get_class($actual), $method));
        }
        $noDeclaredTypeError = \sprintf('Parameter of comparison method %s::%s() does not have a declared type.', \get_class($actual), $method);
        $notAcceptableTypeError = \sprintf('%s is not an accepted argument type for comparison method %s::%s().', \get_class($expected), \get_class($actual), $method);
        $reflParameter = $reflMethod->getParameters()[0];
        if (\method_exists($reflParameter, 'hasType')) {
            // PHP >= 7.0.
            $hasType = $reflParameter->hasType();
            if ($hasType === \false) {
                throw new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Yoast\PHPUnitPolyfills\Exceptions\InvalidComparisonMethodException($noDeclaredTypeError);
            }
            ...
            try {
                ...
                throw new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Yoast\PHPUnitPolyfills\Exceptions\InvalidComparisonMethodException($notAcceptableTypeError);
            }
        ...
        if (\is_bool($result) === \false) {
            throw new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Yoast\PHPUnitPolyfills\Exceptions\InvalidComparisonMethodException(\sprintf('Comparison method %s::%s() does not return a boolean value.', \get_class($actual), $method));
        }
        $msg = \sprintf('Failed asserting that two objects are equal. The objects are not equal according to %s::%s()', \get_class($actual), $method);
        if ($message !== '') {
