...
use WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Yoast\PHPUnitPolyfills\Helpers\ResourceHelper;
/**
...
trait AssertClosedResource
{
    ...
    {
        if ($message === '') {
            $exporter = \class_exists('SebastianBergmann\\Exporter\\Exporter') ? new \SebastianBergmann\Exporter\Exporter() : new \PHPUnit\SebastianBergmann\Exporter\Exporter();
            $message = \sprintf('Failed asserting that %s is of type "resource (closed)"', $exporter->export($actual));
        }
        static::assertTrue(\WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Yoast\PHPUnitPolyfills\Helpers\ResourceHelper::isClosedResource($actual), $message);
    }
    /**
    ...
    public static function assertIsNotClosedResource($actual, $message = '')
    {
        ...
        static::assertFalse(\WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Yoast\PHPUnitPolyfills\Helpers\ResourceHelper::isClosedResource($actual), $message);
    }
    /**
    ...
    {
        return \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Yoast\PHPUnitPolyfills\Helpers\ResourceHelper::isResourceStateReliable($actual) === \false;
    }
}
