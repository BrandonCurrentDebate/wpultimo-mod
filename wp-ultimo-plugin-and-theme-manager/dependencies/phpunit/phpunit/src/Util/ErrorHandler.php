    ...
    public function __invoke(int $errorNumber, string $errorString, string $errorFile, int $errorLine) : bool
    {
        ...
        switch ($errorNumber) {
            ...
            case \E_STRICT:
                ...
                throw new \PHPUnit\Framework\Error\Notice($errorString, $errorNumber, $errorFile, $errorLine);
            case \E_WARNING:
            case \E_USER_WARNING:
                if (!$this->convertWarningsToExceptions) {
                    return \false;
                }
                throw new \PHPUnit\Framework\Error\Warning($errorString, $errorNumber, $errorFile, $errorLine);
            case \E_DEPRECATED:
            case \E_USER_DEPRECATED:
                if (!$this->convertDeprecationsToExceptions) {
                    return \false;
                }
                throw new \PHPUnit\Framework\Error\Deprecated($errorString, $errorNumber, $errorFile, $errorLine);
            default:
                if (!$this->convertErrorsToExceptions) {
                    return \false;
                }
                throw new \PHPUnit\Framework\Error\Error($errorString, $errorNumber, $errorFile, $errorLine);
        }
    }
