...
final class Filter
{
    ...
    private static function shouldPrintFrame($frame, $prefix, \PHPUnit\Util\Blacklist $blacklist) : bool
    {
        ...
        return \is_file($file) && self::fileIsBlacklisted($file, $blacklist) && $fileIsNotPrefixed && $file !== $script;
    }
    private static function fileIsBlacklisted($file, \PHPUnit\Util\Blacklist $blacklist) : bool
    {
        return (empty($GLOBALS['__PHPUNIT_ISOLATION_BLACKLIST']) || !\in_array($file, $GLOBALS['__PHPUNIT_ISOLATION_BLACKLIST'], \true)) && !$blacklist->isBlacklisted($file);
    }
