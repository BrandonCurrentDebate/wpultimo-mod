...
final class Test
{
    ...
    {
        return self::mergeArraysRecursively(\PHPUnit\Util\Annotation\Registry::getInstance()->forClassName($className)->requirements(), \PHPUnit\Util\Annotation\Registry::getInstance()->forMethod($className, $methodName)->requirements());
    }
    /**
    ...
    public static function getMissingRequirements(string $className, string $methodName) : array
    {
        ...
        if (!empty($required['PHP'])) {
            ...
            $version = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PharIo\Version\Version(self::sanitizeVersionNumber(\PHP_VERSION));
        ...
        if (!empty($required['PHPUnit'])) {
            ...
            $phpunitVersion = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PharIo\Version\Version(self::sanitizeVersionNumber(\PHPUnit\Runner\Version::id()));
        ...
        if (!empty($required['OSFAMILY']) && $required['OSFAMILY'] !== (new \SebastianBergmann\Environment\OperatingSystem())->getFamily()) {
    ...
    {
        return \PHPUnit\Util\Annotation\Registry::getInstance()->forMethod($className, $methodName)->expectedException();
    }
    /**
    ...
    {
        return \PHPUnit\Util\Annotation\Registry::getInstance()->forMethod($className, $methodName)->getProvidedData();
    }
    /**
     * @psalm-param class-string $className
     */
    ...
    {
        return \PHPUnit\Util\Annotation\Registry::getInstance()->forMethod($className, $methodName)->getInlineAnnotations();
    }
    /** @psalm-param class-string $className */
        ...
        if (!isset(self::$hookMethods[$className])) {
            ...
            try {
                foreach ((new \ReflectionClass($className))->getMethods() as $method) {
                    ...
                    $docBlock = \PHPUnit\Util\Annotation\Registry::getInstance()->forMethod($className, $method->getName());
                    if ($method->isStatic()) {
    ...
    public static function isTestMethod(\ReflectionMethod $method) : bool
    {
        ...
        return \array_key_exists('test', \PHPUnit\Util\Annotation\Registry::getInstance()->forMethod($method->getDeclaringClass()->getName(), $method->getName())->symbolAnnotations());
    }
    /**
     * @throws CodeCoverageException
     * @psalm-param class-string $className
    ...
    private static function getLinesToBeCoveredOrUsed(string $className, string $methodName, string $mode) : array
    {
        ...
        if (!empty($annotations['class'][$mode . 'DefaultClass'])) {
            if (\count($annotations['class'][$mode . 'DefaultClass']) > 1) {
                throw new \PHPUnit\Framework\CodeCoverageException(\sprintf('More than one @%sClass annotation in class or interface "%s".', $mode, $className));
            }
        ...
        foreach (\array_unique($list) as $element) {
            ...
            if ($mode === 'covers' && \interface_exists($element)) {
                throw new \PHPUnit\Framework\InvalidCoversTargetException(\sprintf('Trying to @cover interface "%s".', $element));
            }
            ...
            if (isset($methodName[0]) && $methodName[0] === '<') {
                ...
                foreach ($classes as $className) {
                    if (!\class_exists($className) && !\interface_exists($className) && !\trait_exists($className)) {
                        throw new \PHPUnit\Framework\InvalidCoversTargetException(\sprintf('Trying to @cover or @use not existing class or ' . 'interface "%s".', $className));
                    }
                    try {
                ...
                foreach ($classes as $className) {
                    if ($className === '' && \function_exists($methodName)) {
                        ...
                        if (!((\class_exists($className) || \interface_exists($className) || \trait_exists($className)) && \method_exists($className, $methodName))) {
                            throw new \PHPUnit\Framework\InvalidCoversTargetException(\sprintf('Trying to @cover or @use not existing method "%s::%s".', $className, $methodName));
                        }
                        try {
