    ...
    public function requirements() : array
    {
        ...
        foreach ($lines as $line) {
            ...
            if (\preg_match(self::REGEX_REQUIRES_VERSION_CONSTRAINT, $line, $matches)) {
                ...
                try {
                    $versionConstraintParser = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PharIo\Version\VersionConstraintParser();
            ...
            if ($data instanceof \Traversable) {
                ...
                foreach ($origData as $key => $value) {
                    if (\is_int($key)) {
                        ...
                        throw new \PHPUnit\Framework\InvalidDataProviderException(\sprintf('The key "%s" has already been defined in the data provider "%s".', $key, $match));
                    } else {
                        $data[$key] = $value;
                    }
                }
    ...
    private function getDataFromTestWithAnnotation(string $docComment) : ?array
    {
        ...
        foreach (\explode("\n", $annotationContent) as $candidateRow) {
            ...
            if (\json_last_error() !== \JSON_ERROR_NONE) {
                throw new \PHPUnit\Util\Exception('The data set for the @testWith annotation cannot be parsed: ' . \json_last_error_msg());
            }
            $data[] = $dataSet;
        }
        if (!$data) {
            throw new \PHPUnit\Util\Exception('The data set for the @testWith annotation cannot be parsed.');
        }
        return $data;
    }
