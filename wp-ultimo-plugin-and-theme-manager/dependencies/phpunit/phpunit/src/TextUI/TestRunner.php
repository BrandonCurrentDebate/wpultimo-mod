...
final class TestRunner extends \PHPUnit\Runner\BaseTestRunner
{
     ...
     * @throws Exception
     */
    public function doRun(\PHPUnit\Framework\Test $suite, array $arguments = [], array $warnings = [], bool $exit = \true) : \PHPUnit\Framework\TestResult
    {
        ...
        if ($arguments['cacheResult']) {
            ...
            $cache = new \PHPUnit\Runner\DefaultTestResultCache($arguments['cacheResultFile']);
        ...
        if ($this->printer === null) {
            if (isset($arguments['printer'])) {
                if ($arguments['printer'] instanceof \PHPUnit\Util\Printer && $arguments['printer'] instanceof \PHPUnit\Framework\TestListener) {
                    ...
                    if (\is_subclass_of($arguments['printer'], \PHPUnit\TextUI\ResultPrinter::class)) {
                        $this->printer = $this->createPrinter($arguments['printer'], $arguments);
                    }
                }
            } else {
                $this->printer = $this->createPrinter(\PHPUnit\TextUI\ResultPrinter::class, $arguments);
            }
        }
        ...
        if (isset($arguments['xdebugFilterFile'], $filterConfiguration)) {
            $this->write("\n");
            $script = (new \PHPUnit\Util\XdebugFilterScriptGenerator())->generate($filterConfiguration['whitelist']);
            if ($arguments['xdebugFilterFile'] !== 'php://stdout' && $arguments['xdebugFilterFile'] !== 'php://stderr' && !\PHPUnit\Util\Filesystem::createDirectory(\dirname($arguments['xdebugFilterFile']))) {
        ...
        if (isset($arguments['configuration'])) {
            ...
            foreach ($arguments['configuration']->getExtensionConfiguration() as $extension) {
                ...
                if (\count($extension['arguments']) === 0) {
                    ...
                    $extensionObject = $extensionClass->newInstanceArgs($extension['arguments']);
                }
                \assert($extensionObject instanceof \PHPUnit\Runner\Hook);
    ...
    {
        return new $class(isset($arguments['stderr']) && $arguments['stderr'] === \true ? 'php://stderr' : null, $arguments['verbose'], $arguments['colors'], $arguments['debug'], $arguments['columns'], $arguments['reverseList']);
    }
    ...
    {
        $this->printer->write(\sprintf("done [%s]\n", \SebastianBergmann\Timer\Timer::secondsToTimeString(\SebastianBergmann\Timer\Timer::stop())));
    }
    private function codeCoverageGenerationFailed(\Exception $e) : void
    {
        $this->printer->write(\sprintf("failed [%s]\n%s\n", \SebastianBergmann\Timer\Timer::secondsToTimeString(\SebastianBergmann\Timer\Timer::stop()), $e->getMessage()));
    }
}
