...
use WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PharIo\Manifest\ApplicationName;
use WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PharIo\Manifest\Exception as ManifestException;
use WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PharIo\Manifest\ManifestLoader;
...
class Command
{
    ...
    public function run(array $argv, bool $exit = \true) : int
    {
        ...
        if ($this->arguments['test'] instanceof \PHPUnit\Framework\Test) {
    ...
    {
        try {
            $this->options = \PHPUnit\Util\Getopt::parse($argv, 'd:c:hv', \array_keys($this->longOptions));
        } catch (\PHPUnit\Framework\Exception $t) {
        ...
        foreach ($this->options[0] as $option) {
            switch ($option[0]) {
                ...
                case '--bootstrap':
                    ...
                    break;
                case '--cache-result':
                    $this->arguments['cacheResult'] = \true;
                    break;
                ...
                case '--generate-configuration':
                    ...
                    \file_put_contents('phpunit.xml', $generator->generateDefaultConfiguration(\PHPUnit\Runner\Version::series(), $bootstrapScript, $testsDirectory, $src));
                ...
                case '--stderr':
                    ...
                    break;
                case '--stop-on-defect':
                    $this->arguments['stopOnDefect'] = \true;
                    break;
                case '--stop-on-error':
                ...
                case '--fail-on-risky':
                    ...
                    break;
                case '--teamcity':
                    $this->arguments['printer'] = \PHPUnit\Util\Log\TeamCity::class;
                    break;
                case '--testdox':
                    $this->arguments['printer'] = \PHPUnit\Util\TestDox\CliTestDoxPrinter::class;
                    break;
                case '--testdox-group':
                ...
                case '--no-logging':
                    ...
                    break;
                case '--no-interaction':
                    $this->arguments['noInteraction'] = \true;
                    break;
                case '--globals-backup':
                ...
                case '--atleast-version':
                    if (\version_compare(\PHPUnit\Runner\Version::id(), $option[1], '>=')) {
                        exit(\PHPUnit\TextUI\TestRunner::SUCCESS_EXIT);
                    }
        ...
        if (isset($this->arguments['configuration'])) {
            try {
                $configuration = \PHPUnit\Util\Configuration::getInstance($this->arguments['configuration']);
            } catch (\Throwable $t) {
    ...
    protected function handleLoader(string $loaderClass, string $loaderFile = '') : ?\PHPUnit\Runner\TestSuiteLoader
    {
        if (!\class_exists($loaderClass, \false)) {
            if ($loaderFile == '') {
                $loaderFile = \PHPUnit\Util\Filesystem::classNameToFilename($loaderClass);
            }
        ...
        if (\class_exists($loaderClass, \false)) {
            ...
            if ($class->implementsInterface(\PHPUnit\Runner\TestSuiteLoader::class) && $class->isInstantiable()) {
                $object = $class->newInstance();
                \assert($object instanceof \PHPUnit\Runner\TestSuiteLoader);
                return $object;
            }
        }
        if ($loaderClass == \PHPUnit\Runner\StandardTestSuiteLoader::class) {
            return null;
        }
    ...
    protected function handlePrinter(string $printerClass, string $printerFile = '')
    {
        ...
        if (!$class->implementsInterface(\PHPUnit\Framework\TestListener::class)) {
            $this->exitWithErrorMessage(\sprintf('Could not use "%s" as printer: class does not implement %s', $printerClass, \PHPUnit\Framework\TestListener::class));
        }
        if (!$class->isSubclassOf(\PHPUnit\Util\Printer::class)) {
            $this->exitWithErrorMessage(\sprintf('Could not use "%s" as printer: class does not extend %s', $printerClass, \PHPUnit\Util\Printer::class));
        }
    ...
    {
        foreach ((new \SebastianBergmann\FileIterator\Facade())->getFilesAsArray($directory, '.phar') as $file) {
            if (!\file_exists('phar://' . $file . '/manifest.xml')) {
                $this->arguments['notLoadedExtensions'][] = $file . ' is not an extension for PHPUnit';
                continue;
            }
            try {
                $applicationName = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PharIo\Manifest\ApplicationName('phpunit/phpunit');
                $version = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PharIo\Version\Version(\PHPUnit\Runner\Version::series());
                $manifest = \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PharIo\Manifest\ManifestLoader::fromFile('phar://' . $file . '/manifest.xml');
    ...
    private function handleOrderByOption(string $value) : void
    {
        foreach (\explode(',', $value) as $order) {
            switch ($order) {
                case 'default':
                    $this->arguments['executionOrder'] = \PHPUnit\Runner\TestSuiteSorter::ORDER_DEFAULT;
                    $this->arguments['executionOrderDefects'] = \PHPUnit\Runner\TestSuiteSorter::ORDER_DEFAULT;
                    $this->arguments['resolveDependencies'] = \true;
                    break;
                case 'defects':
                    $this->arguments['executionOrderDefects'] = \PHPUnit\Runner\TestSuiteSorter::ORDER_DEFECTS_FIRST;
                    break;
                case 'depends':
                    $this->arguments['resolveDependencies'] = \true;
                    break;
                case 'duration':
                    $this->arguments['executionOrder'] = \PHPUnit\Runner\TestSuiteSorter::ORDER_DURATION;
                    break;
                case 'no-depends':
                    $this->arguments['resolveDependencies'] = \false;
                    break;
                case 'random':
                    $this->arguments['executionOrder'] = \PHPUnit\Runner\TestSuiteSorter::ORDER_RANDOMIZED;
                    break;
                case 'reverse':
                    $this->arguments['executionOrder'] = \PHPUnit\Runner\TestSuiteSorter::ORDER_REVERSED;
                    break;
                case 'size':
                    $this->arguments['executionOrder'] = \PHPUnit\Runner\TestSuiteSorter::ORDER_SIZE;
                    break;
                default:
