...
final class StandardTestSuiteLoader implements \PHPUnit\Runner\TestSuiteLoader
{
    ...
    public function load(string $suiteClassName, string $suiteClassFile = '') : \ReflectionClass
    {
        ...
        if (empty($suiteClassFile)) {
            $suiteClassFile = \PHPUnit\Util\Filesystem::classNameToFilename($suiteClassName);
        }
        ...
        if (!empty($loadedClasses) && !\class_exists($suiteClassName, \false)) {
            $testCaseClass = \PHPUnit\Framework\TestCase::class;
            foreach ($loadedClasses as $loadedClass) {
                try {
                    $class = new \ReflectionClass($loadedClass);
                    // @codeCoverageIgnoreStart
        ...
        throw new \PHPUnit\Runner\Exception(\sprintf("Class '%s' could not be found in '%s'.", $suiteClassName, $suiteClassFile));
    }
