...
class TestSuite implements \IteratorAggregate, \PHPUnit\Framework\SelfDescribing, \PHPUnit\Framework\Test
{
    ...
    public function addTestSuite($testClass) : void
    {
        ...
        if ($testClass instanceof self) {
            ...
            $suiteMethod = \false;
            if (!$testClass->isAbstract() && $testClass->hasMethod(\PHPUnit\Runner\BaseTestRunner::SUITE_METHODNAME)) {
                try {
                    $method = $testClass->getMethod(\PHPUnit\Runner\BaseTestRunner::SUITE_METHODNAME);
                    // @codeCoverageIgnoreStart
                } catch (\ReflectionException $e) {
                    throw new \PHPUnit\Framework\Exception($e->getMessage(), (int) $e->getCode(), $e);
                }
        ...
        foreach ($newClasses as $className) {
            ...
            if (!$class->isAbstract()) {
                if ($class->hasMethod(\PHPUnit\Runner\BaseTestRunner::SUITE_METHODNAME)) {
                    try {
                        $method = $class->getMethod(\PHPUnit\Runner\BaseTestRunner::SUITE_METHODNAME);
                        // @codeCoverageIgnoreStart
                    } catch (\ReflectionException $e) {
                        throw new \PHPUnit\Framework\Exception($e->getMessage(), (int) $e->getCode(), $e);
                    }
    ...
    public function run(\PHPUnit\Framework\TestResult $result = null) : \PHPUnit\Framework\TestResult
    {
        ...
        $className = $this->name;
        $hookMethods = \PHPUnit\Util\Test::getHookMethods($className);
        $result->startTestSuite($this);
        try {
            foreach ($hookMethods['beforeClass'] as $beforeClassMethod) {
                if ($this->testCase && \class_exists($this->name, \false) && \method_exists($this->name, $beforeClassMethod)) {
        ...
        try {
            foreach ($hookMethods['afterClass'] as $afterClassMethod) {
                if ($this->testCase && \class_exists($this->name, \false) && \method_exists($this->name, $afterClassMethod)) {
    ...
    protected function addTestMethod(\ReflectionClass $class, \ReflectionMethod $method) : void
    {
        ...
        $this->addTest($test, \PHPUnit\Util\Test::getGroups($class->getName(), $methodName));
    }
}
