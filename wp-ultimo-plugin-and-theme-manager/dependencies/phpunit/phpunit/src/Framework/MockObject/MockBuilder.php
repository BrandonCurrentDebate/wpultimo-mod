...
final class MockBuilder
{
    ...
    public function onlyMethods(array $methods) : self
    {
        ...
        foreach ($methods as $method) {
            if (!$reflector->hasMethod($method)) {
                throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Trying to set mock method "%s" with onlyMethods, but it does not exist in class "%s". Use addMethods() for methods that don\'t exist in the class.', $method, $this->type));
            }
        }
        $this->methods = \array_merge($this->methods ?? [], $methods);
        return $this;
    ...
    public function addMethods(array $methods) : self
    {
        ...
        foreach ($methods as $method) {
            if ($reflector->hasMethod($method)) {
                throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Trying to set mock method "%s" with addMethods(), but it exists in class "%s". Use onlyMethods() for methods that exist in the class.', $method, $this->type));
            }
        }
        $this->methods = \array_merge($this->methods ?? [], $methods);
        return $this;
