...
final class InvocationHandler
{
    ...
    {
        if (isset($this->matcherMap[$id])) {
            throw new \PHPUnit\Framework\MockObject\RuntimeException('Matcher with id <' . $id . '> is already registered.');
        }
    ...
    public function invoke(\PHPUnit\Framework\MockObject\Invocation $invocation)
    {
        ...
        if (!$this->returnValueGeneration) {
            $exception = new \PHPUnit\Framework\ExpectationFailedException(\sprintf('Return value inference disabled and no expectation set up for %s::%s()', $invocation->getClassName(), $invocation->getMethodName()));
