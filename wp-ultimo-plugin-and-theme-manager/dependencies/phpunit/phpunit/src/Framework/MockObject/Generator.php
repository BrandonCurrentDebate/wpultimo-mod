...
use WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Doctrine\Instantiator\Exception\ExceptionInterface as InstantiatorException;
...
final class Generator
{
    ...
    public function getMock($type, $methods = [], array $arguments = [], string $mockClassName = '', bool $callOriginalConstructor = \true, bool $callOriginalClone = \true, bool $callAutoload = \true, bool $cloneArguments = \true, bool $callOriginalMethods = \false, object $proxyTarget = null, bool $allowMockingUnknownTypes = \true, bool $returnValueGeneration = \true) : \PHPUnit\Framework\MockObject\MockObject
    {
        if (!\is_array($type) && !\is_string($type)) {
            throw \PHPUnit\Framework\InvalidArgumentException::create(1, 'array or string');
        }
        ...
        if (\is_array($type)) {
            $type = \array_unique(\array_map(static function ($type) {
            ...
            if (\is_array($type)) {
                foreach ($type as $_type) {
                    if (!\class_exists($_type, $callAutoload) && !\interface_exists($_type, $callAutoload)) {
                        throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Cannot stub or mock class or interface "%s" which does not exist', $_type));
                    }
                }
            } elseif (!\class_exists($type, $callAutoload) && !\interface_exists($type, $callAutoload)) {
                throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Cannot stub or mock class or interface "%s" which does not exist', $type));
            }
        }
        if (null !== $methods) {
            foreach ($methods as $method) {
                if (!\preg_match('~[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*~', (string) $method)) {
                    throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Cannot stub or mock method with invalid name "%s"', $method));
                }
            }
            if ($methods !== \array_unique($methods)) {
                throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Cannot stub or mock using a method list that contains duplicates: "%s" (duplicate: "%s")', \implode(', ', $methods), \implode(', ', \array_unique(\array_diff_assoc($methods, \array_unique($methods))))));
            }
        }
        if ($mockClassName !== '' && \class_exists($mockClassName, \false)) {
            ...
            if (!$reflector->implementsInterface(\PHPUnit\Framework\MockObject\MockObject::class)) {
                throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Class "%s" already exists.', $mockClassName));
            }
        }
        if (!$callOriginalConstructor && $callOriginalMethods) {
            throw new \PHPUnit\Framework\MockObject\RuntimeException('Proxying to original methods requires invoking the original constructor');
        }
    ...
    public function getMockForAbstractClass(string $originalClassName, array $arguments = [], string $mockClassName = '', bool $callOriginalConstructor = \true, bool $callOriginalClone = \true, bool $callAutoload = \true, array $mockedMethods = null, bool $cloneArguments = \true) : \PHPUnit\Framework\MockObject\MockObject
    {
        ...
        throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Class "%s" does not exist.', $originalClassName));
    }
    /**
    ...
    public function getMockForTrait(string $traitName, array $arguments = [], string $mockClassName = '', bool $callOriginalConstructor = \true, bool $callOriginalClone = \true, bool $callAutoload = \true, array $mockedMethods = null, bool $cloneArguments = \true) : \PHPUnit\Framework\MockObject\MockObject
    {
        if (!\trait_exists($traitName, $callAutoload)) {
            throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Trait "%s" does not exist.', $traitName));
        }
    ...
    public function generate($type, array $methods = null, string $mockClassName = '', bool $callOriginalClone = \true, bool $callAutoload = \true, bool $cloneArguments = \true, bool $callOriginalMethods = \false) : \PHPUnit\Framework\MockObject\MockClass
    {
        if (\is_array($type)) {
            \sort($type);
        }
    ...
    public function generateClassFromWsdl(string $wsdlFile, string $className, array $methods = [], array $options = []) : string
    {
        if (!\extension_loaded('soap')) {
            throw new \PHPUnit\Framework\MockObject\RuntimeException('The SOAP extension is required to generate a mock object from WSDL.');
        }
    ...
    private function getObject(\PHPUnit\Framework\MockObject\MockType $mockClass, $type = '', bool $callOriginalConstructor = \false, bool $callAutoload = \false, array $arguments = [], bool $callOriginalMethods = \false, object $proxyTarget = null, bool $returnValueGeneration = \true)
    {
        ...
        if ($callOriginalConstructor) {
            ...
            try {
                $object = (new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Doctrine\Instantiator\Instantiator())->instantiate($className);
            } catch (\WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Doctrine\Instantiator\Exception\ExceptionInterface $exception) {
                throw new \PHPUnit\Framework\MockObject\RuntimeException($exception->getMessage());
            }
        }
    ...
    private function generateMock($type, ?array $explicitMethods, string $mockClassName, bool $callOriginalClone, bool $callAutoload, bool $cloneArguments, bool $callOriginalMethods) : \PHPUnit\Framework\MockObject\MockClass
    {
        ...
        if (\is_array($type)) {
            ...
            foreach ($type as $_type) {
                if (!\interface_exists($_type, $callAutoload)) {
                    throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Interface "%s" does not exist.', $_type));
                }
                ...
                foreach ($this->getClassMethods($_type) as $method) {
                    if (\in_array($method, $interfaceMethods, \true)) {
                        throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Duplicate method "%s" not allowed.', $method));
                    }
                    try {
                    ...
                    if ($this->canMockMethod($methodReflection)) {
                        $mockMethods->addMethods(\PHPUnit\Framework\MockObject\MockMethod::fromReflection($methodReflection, $callOriginalMethods, $cloneArguments));
        ...
        if (!$isClass && !$isInterface) {
            ...
            // @codeCoverageIgnoreEnd
            if ($class->isFinal()) {
                throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Class "%s" is declared "final" and cannot be mocked.', $mockClassName['fullClassName']));
            }
        ...
        if ($isInterface && ($explicitMethods === [] || $explicitMethods === null)) {
            $mockMethods->addMethods(...$this->mockInterfaceMethods($mockClassName['fullClassName'], $cloneArguments));
        }
        if (\is_array($explicitMethods)) {
            foreach ($explicitMethods as $methodName) {
                if ($class !== null && $class->hasMethod($methodName)) {
                    ...
                    $mockMethods->addMethods(\PHPUnit\Framework\MockObject\MockMethod::fromName($mockClassName['fullClassName'], $methodName, $cloneArguments));
                }
            }
        }
        $mockedMethods = '';
        ...
        $classTemplate->setVar(['prologue' => $prologue ?? '', 'epilogue' => $epilogue ?? '', 'class_declaration' => $this->generateMockClassDeclaration($mockClassName, $isInterface, $additionalInterfaces), 'clone' => $cloneTrait, 'mock_class_name' => $mockClassName['className'], 'mocked_methods' => $mockedMethods, 'method' => $method]);
        return new \PHPUnit\Framework\MockObject\MockClass($classTemplate->render(), $mockClassName['className'], $configurable);
    }
    /**
     * @param array|string $type
     */
    ...
    private function getTemplate(string $template) : \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Text_Template
    {
        $filename = __DIR__ . \DIRECTORY_SEPARATOR . 'Generator' . \DIRECTORY_SEPARATOR . $template;
        if (!isset(self::$templates[$filename])) {
            self::$templates[$filename] = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Text_Template($filename);
        }
