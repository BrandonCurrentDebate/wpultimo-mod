...
final class MockMethod
{
    ...
    public static function fromReflection(\ReflectionMethod $method, bool $callOriginalMethod, bool $cloneArguments) : self
    {
        ...
        return new self($method->getDeclaringClass()->getName(), $method->getName(), $cloneArguments, $modifier, self::getMethodParametersForDeclaration($method), self::getMethodParametersForCall($method), self::deriveReturnType($method), $reference, $callOriginalMethod, $method->isStatic(), $deprecation);
    }
    public static function fromName(string $fullClassName, string $methodName, bool $cloneArguments) : self
    {
        return new self($fullClassName, $methodName, $cloneArguments, 'public', '', '', new \SebastianBergmann\Type\UnknownType(), '', \false, \false, null);
    }
    ...
    public function generateCode() : string
    {
        ...
        } elseif ($this->returnType instanceof \SebastianBergmann\Type\VoidType) {
         ...
         */
        $returnTypeDeclaration = \str_replace('?mixed', 'mixed', $this->returnType->getReturnTypeDeclaration());
        $template = $this->getTemplate($templateFile);
        $template->setVar(['arguments_decl' => $this->argumentsForDeclaration, 'arguments_call' => $this->argumentsForCall, 'return_declaration' => $returnTypeDeclaration, 'arguments_count' => !empty($this->argumentsForCall) ? \substr_count($this->argumentsForCall, ',') + 1 : 0, 'class_name' => $this->className, 'method_name' => $this->methodName, 'modifier' => $this->modifier, 'reference' => $this->reference, 'clone_arguments' => $this->cloneArguments ? 'true' : 'false', 'deprecation' => $deprecation]);
    ...
    private static function getMethodParametersForDeclaration(\ReflectionMethod $method) : string
    {
        ...
        foreach ($method->getParameters() as $i => $parameter) {
            ...
            if ($parameter->isVariadic()) {
                ...
                $default = ' = ' . \var_export($parameter->getDefaultValue(), \true);
    ...
    private static function deriveReturnType(\ReflectionMethod $method) : \SebastianBergmann\Type\Type
    {
        ...
        if ($returnType instanceof \ReflectionNamedType && $returnType->getName() === 'self') {
            return \SebastianBergmann\Type\ObjectType::fromName($method->getDeclaringClass()->getName(), $returnType->allowsNull());
        }
        ...
        if ($returnType instanceof \ReflectionNamedType && $returnType->getName() === 'parent') {
            ...
            if ($parentClass === \false) {
                throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Cannot mock %s::%s because "parent" return type declaration is used but %s does not have a parent class', $method->getDeclaringClass()->getName(), $method->getName(), $method->getDeclaringClass()->getName()));
            }
            return \SebastianBergmann\Type\ObjectType::fromName($parentClass->getName(), $returnType->allowsNull());
        }
        return \SebastianBergmann\Type\Type::fromName($returnType->getName(), $returnType->allowsNull());
    }
