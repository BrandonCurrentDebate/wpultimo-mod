...
use Throwable;
abstract class TestCase extends \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\assert implements \PHPUnit\Framework\SelfDescribing, \PHPUnit\Framework\Test
{
    ...
    {
        $this->deprecatedExpectExceptionMessageRegExpUsed = \true;
    ...
    public function hasFailed() : bool
    {
        ...
        return $status === \PHPUnit\Runner\BaseTestRunner::STATUS_FAILURE || $status === \PHPUnit\Runner\BaseTestRunner::STATUS_ERROR;
    }
    /**
    ...
    public function run(\PHPUnit\Framework\TestResult $result = null) : \PHPUnit\Framework\TestResult
    {
        ...
        if (!$this instanceof \PHPUnit\Framework\WarningTestCase && !$this instanceof \PHPUnit\Framework\SkippedTestCase && !$this->handleDependencies()) {
            return $result;
        }
        if ($this->runInSeparateProcess()) {
            ...
            // @codeCoverageIgnoreEnd
            if ($runEntireClass) {
                $template = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Text_Template(__DIR__ . '/../Util/PHP/Template/TestCaseClass.tpl');
            } else {
                $template = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Text_Template(__DIR__ . '/../Util/PHP/Template/TestCaseMethod.tpl');
            }
    ...
    {
        \SebastianBergmann\Comparator\Factory::getInstance()->register($comparator);
    ...
    {
        return \PHPUnit\Util\Test::parseTestMethodAnnotations(static::class, $this->name);
    }
    /**
     * @throws \SebastianBergmann\RecursionContext\InvalidArgumentException
     *
    ...
    {
        return \PHPUnit\Util\Test::getSize(static::class, $this->getName(\false));
    }
    /**
     * @throws \SebastianBergmann\RecursionContext\InvalidArgumentException
     *
    ...
    public function runBare() : void
    {
        ...
        $hookMethods = \PHPUnit\Util\Test::getHookMethods(static::class);
        ...
        if (isset($e)) {
            if ($e instanceof \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Exception\Prediction\PredictionException) {
    ...
    protected function runTest()
    {
        ...
        try {
            ...
            if ($this->expectedException !== null) {
                if ($this->expectedException === \PHPUnit\Framework\Error\Error::class) {
                    $this->assertThat($exception, \PHPUnit\Framework\Constraint\LogicalOr::fromConstraints(new \PHPUnit\Framework\Constraint\Exception(\PHPUnit\Framework\Error\Error::class), new \PHPUnit\Framework\Constraint\Exception(\Error::class)));
                } else {
    ...
    protected function createPartialMock($originalClassName, array $methods) : \PHPUnit\Framework\MockObject\MockObject
    {
        ...
        foreach ($class_names as $class_name) {
            ...
            $mockedMethodsThatDontExist = \array_filter($methods, static function (string $method) use($reflection) {
                return !$reflection->hasMethod($method);
            });
            if ($mockedMethodsThatDontExist) {
                $this->addWarning(\sprintf('createPartialMock called with method(s) %s that do not exist in %s. This will not be allowed in future versions of PHPUnit.', \implode(', ', $mockedMethodsThatDontExist), $class_name));
            }
        }
     ...
     * @psalm-return MockObject&RealInstanceType
     */
    protected function getMockForAbstractClass($originalClassName, array $arguments = [], $mockClassName = '', $callOriginalConstructor = \true, $callOriginalClone = \true, $callAutoload = \true, $mockedMethods = [], $cloneArguments = \false) : \PHPUnit\Framework\MockObject\MockObject
    {
        $this->recordDoubledType($originalClassName);
     ...
     * @psalm-return MockObject&RealInstanceType
     */
    protected function getMockFromWsdl($wsdlFile, $originalClassName = '', $mockClassName = '', array $methods = [], $callOriginalConstructor = \true, array $options = []) : \PHPUnit\Framework\MockObject\MockObject
    {
     ...
     * @param bool   $cloneArguments
     */
    protected function getMockForTrait($traitName, array $arguments = [], $mockClassName = '', $callOriginalConstructor = \true, $callOriginalClone = \true, $callAutoload = \true, $mockedMethods = [], $cloneArguments = \false) : \PHPUnit\Framework\MockObject\MockObject
    {
     ...
     */
    protected function prophesize($classOrInterface = null) : \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Prophecy\ObjectProphecy
    {
    ...
    private function setExpectedExceptionFromAnnotation() : void
    {
        ...
        try {
            $expectedException = \PHPUnit\Util\Test::getExpectedException(static::class, $this->name);
    ...
    private function checkRequirements() : void
    {
        ...
        $missingRequirements = \PHPUnit\Util\Test::getMissingRequirements(static::class, $this->name);
                ...
                foreach ($this->prophet->getProphecies() as $objectProphecy) {
                    foreach ($objectProphecy->getMethodProphecies() as $methodProphecies) {
                        foreach ($methodProphecies as $methodProphecy) {
                            \assert($methodProphecy instanceof \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Prophecy\MethodProphecy);
        ...
        if (!empty($this->dependencies) && !$this->inIsolation) {
            ...
            foreach ($this->dependencies as $dependency) {
                ...
                if (isset($passed[$dependency])) {
                    ...
                    if ($deepClone) {
                        $deepCopy = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\DeepCopy\DeepCopy();
    ...
    private function markSkippedForMissingDependency(string $dependency) : void
    {
        $this->status = \PHPUnit\Runner\BaseTestRunner::STATUS_SKIPPED;
        $this->result->startTest($this);
        $this->result->addError($this, new \PHPUnit\Framework\SkippedTestError(\sprintf('This test depends on "%s" to pass.', $dependency)), 0);
        $this->result->endTest($this, 0);
    }
    private function warnAboutDependencyThatDoesNotExist(string $dependency) : void
    {
        $this->status = \PHPUnit\Runner\BaseTestRunner::STATUS_WARNING;
        $this->result->startTest($this);
        $this->result->addWarning($this, new \PHPUnit\Framework\Warning(\sprintf('This test depends on "%s" which does not exist.', $dependency)), 0);
        $this->result->endTest($this, 0);
    }
    /**
    ...
    {
        if (!$this->snapshot instanceof \SebastianBergmann\GlobalState\Snapshot) {
            return;
        }
    ...
    {
        $blacklist = new \SebastianBergmann\GlobalState\Blacklist();
        ...
        if (!\defined('PHPUNIT_TESTSUITE')) {
            ...
            $blacklist->addClassNamePrefix('WP_Ultimo_Plugin_And_Theme_Manager\\Dependencies\\Doctrine\\Instantiator');
            $blacklist->addClassNamePrefix('Prophecy');
            $blacklist->addStaticAttribute(\SebastianBergmann\Comparator\Factory::class, 'instance');
        ...
        return new \SebastianBergmann\GlobalState\Snapshot($blacklist, $backupGlobals, (bool) $this->backupStaticAttributes, \false, \false, \false, \false, \false, \false, \false);
    }
    /**
     * @throws \SebastianBergmann\RecursionContext\InvalidArgumentException
     * @throws RiskyTestError
    ...
    private function getProphet() : \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Prophet
    {
        if ($this->prophet === null) {
            $this->prophet = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Prophet();
        }
        return $this->prophet;
    }
    /**
    ...
    {
        $enumerator = new \SebastianBergmann\ObjectEnumerator\Enumerator();
    ...
    private function registerMockObjectsFromTestArguments(array $testArguments, array &$visited = []) : void
    {
        if ($this->registerMockObjectsFromTestArgumentsRecursively) {
            foreach ((new \SebastianBergmann\ObjectEnumerator\Enumerator())->enumerate($testArguments) as $object) {
    ...
    {
        $factory = \SebastianBergmann\Comparator\Factory::getInstance();
    ...
    private function isCallableTestMethod(string $dependency) : bool
    {
        ...
        return \PHPUnit\Util\Test::isTestMethod($method);
    }
}
