...
final class File extends \SebastianBergmann\CodeCoverage\Report\Html\Renderer
{
    ...
    {
        $template = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Text_Template($this->templatePath . 'file.html', '{{', '}}');
        $template->setVar(['items' => $this->renderItems($node), 'lines' => $this->renderSource($node)]);
        $this->setCommonTemplateVariables($template, $node);
        $template->renderTo($file);
    }
    protected function renderItems(\SebastianBergmann\CodeCoverage\Node\File $node) : string
    {
        $template = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Text_Template($this->templatePath . 'file_item.html', '{{', '}}');
        $methodItemTemplate = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Text_Template($this->templatePath . 'method_item.html', '{{', '}}');
    ...
    protected function renderTraitOrClassItems(array $items, \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Text_Template $template, \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Text_Template $methodItemTemplate) : string
    {
        $buffer = '';
        if (empty($items)) {
            return $buffer;
    ...
    protected function renderFunctionItems(array $functions, \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Text_Template $template) : string
    {
        if (empty($functions)) {
            return '';
        }
    ...
    protected function renderFunctionOrMethodItem(\WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Text_Template $template, array $item, string $indent = '') : string
    {
        $numMethods = 0;
        $numTestedMethods = 0;
        if ($item['executableLines'] > 0) {
        ...
        return $this->renderItemTemplate($template, ['name' => \sprintf('%s<a href="#%d"><abbr title="%s">%s</abbr></a>', $indent, $item['startLine'], \htmlspecialchars($item['signature'], $this->htmlSpecialCharsFlags), $item['functionName'] ?? $item['methodName']), 'numMethods' => $numMethods, 'numTestedMethods' => $numTestedMethods, 'linesExecutedPercent' => \SebastianBergmann\CodeCoverage\Util::percent($item['executedLines'], $item['executableLines']), 'linesExecutedPercentAsString' => \SebastianBergmann\CodeCoverage\Util::percent($item['executedLines'], $item['executableLines'], \true), 'numExecutedLines' => $item['executedLines'], 'numExecutableLines' => $item['executableLines'], 'testedMethodsPercent' => \SebastianBergmann\CodeCoverage\Util::percent($numTestedMethods, 1), 'testedMethodsPercentAsString' => \SebastianBergmann\CodeCoverage\Util::percent($numTestedMethods, 1, \true), 'crap' => $item['crap']]);
    }
    protected function renderSource(\SebastianBergmann\CodeCoverage\Node\File $node) : string
    {
        ...
        foreach ($codeLines as $line) {
            ...
            $lines .= \sprintf('     <tr%s><td%s><div align="right"><a name="%d"></a><a href="#%d">%d</a></div></td><td class="codeLine">%s</td></tr>' . "\n", $trClass, $popover, $i, $i, $i, $line);
            $i++;
        }
        return $lines;
    }
