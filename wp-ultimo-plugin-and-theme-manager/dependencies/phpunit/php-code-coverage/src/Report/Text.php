...
final class Text
{
    ...
    public function process(\SebastianBergmann\CodeCoverage\CodeCoverage $coverage, bool $showColors = \false) : string
    {
        ...
        $classes = \sprintf('  Classes: %6s (%d/%d)', \SebastianBergmann\CodeCoverage\Util::percent($report->getNumTestedClassesAndTraits(), $report->getNumClassesAndTraits(), \true), $report->getNumTestedClassesAndTraits(), $report->getNumClassesAndTraits());
        $methods = \sprintf('  Methods: %6s (%d/%d)', \SebastianBergmann\CodeCoverage\Util::percent($report->getNumTestedMethods(), $report->getNumMethods(), \true), $report->getNumTestedMethods(), $report->getNumMethods());
        $lines = \sprintf('  Lines:   %6s (%d/%d)', \SebastianBergmann\CodeCoverage\Util::percent($report->getNumExecutedLines(), $report->getNumExecutableLines(), \true), $report->getNumExecutedLines(), $report->getNumExecutableLines());
    ...
    {
        $coverage = \SebastianBergmann\CodeCoverage\Util::percent($numberOfCoveredElements, $totalNumberOfElements);
    ...
    {
        $format = '%' . $precision . 's';
        return \SebastianBergmann\CodeCoverage\Util::percent($numberOfCoveredElements, $totalNumberOfElements, \true, \true) . ' (' . \sprintf($format, $numberOfCoveredElements) . '/' . \sprintf($format, $totalNumberOfElements) . ')';
    }
