...
final class Facade
{
    ...
    public function process(\SebastianBergmann\CodeCoverage\CodeCoverage $coverage, string $target) : void
    {
        ...
        $this->project = new \SebastianBergmann\CodeCoverage\Report\Xml\Project($coverage->getReport()->getName());
    ...
    private function setBuildInformation() : void
    {
        ...
        $buildNode->setRuntimeInformation(new \SebastianBergmann\Environment\Runtime());
        $buildNode->setBuildTime(\DateTime::createFromFormat('U', (string) $_SERVER['REQUEST_TIME']));
        $buildNode->setGeneratorVersions($this->phpUnitVersion, \SebastianBergmann\CodeCoverage\Version::id());
    }
    /**
     * @throws RuntimeException
     */
    private function initTargetDirectory(string $directory) : void
    {
        if (\file_exists($directory)) {
            if (!\is_dir($directory)) {
                throw new \SebastianBergmann\CodeCoverage\RuntimeException("'{$directory}' exists but is not a directory.");
            }
            if (!\is_writable($directory)) {
                throw new \SebastianBergmann\CodeCoverage\RuntimeException("'{$directory}' exists but is not writable.");
            }
        } elseif (!$this->createDirectory($directory)) {
            throw new \SebastianBergmann\CodeCoverage\RuntimeException("'{$directory}' could not be created.");
        }
    }
    ...
    private function processFile(\SebastianBergmann\CodeCoverage\Node\File $file, \SebastianBergmann\CodeCoverage\Report\Xml\Directory $context) : void
    {
        ...
        $fileReport = new \SebastianBergmann\CodeCoverage\Report\Xml\Report($path);
