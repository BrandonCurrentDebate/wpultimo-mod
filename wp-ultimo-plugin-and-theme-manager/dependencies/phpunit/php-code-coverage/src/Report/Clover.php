    ...
    public function process(\SebastianBergmann\CodeCoverage\CodeCoverage $coverage, ?string $target = null, ?string $name = null) : string
    {
        ...
        foreach ($report as $item) {
            ...
            foreach ($classes as $className => $class) {
                ...
                $xmlMetrics->setAttribute('elements', (string) ($classMethods + $classStatements));
                $xmlMetrics->setAttribute('coveredelements', (string) ($coveredMethods + $coveredClassStatements));
            ...
            $xmlMetrics->setAttribute('elements', (string) ($item->getNumMethods() + $item->getNumExecutableLines()));
            $xmlMetrics->setAttribute('coveredelements', (string) ($item->getNumTestedMethods() + $item->getNumExecutedLines()));
        ...
        $xmlMetrics->setAttribute('elements', (string) ($report->getNumMethods() + $report->getNumExecutableLines()));
        $xmlMetrics->setAttribute('coveredelements', (string) ($report->getNumTestedMethods() + $report->getNumExecutedLines()));
        ...
        if ($target !== null) {
            ...
            if (@\file_put_contents($target, $buffer) === \false) {
                throw new \SebastianBergmann\CodeCoverage\RuntimeException(\sprintf('Could not write to "%s', $target));
            }
        }
        return $buffer;
    }
