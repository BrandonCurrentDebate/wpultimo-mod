...
final class CodeCoverage
{
     ...
     */
    public function __construct(\SebastianBergmann\CodeCoverage\Driver\Driver $driver = null, \SebastianBergmann\CodeCoverage\Filter $filter = null)
    {
        if ($filter === null) {
        ...
        $this->driver = $driver;
        $this->filter = $filter;
        $this->wizard = new \SebastianBergmann\CodeUnitReverseLookup\Wizard();
    }
    /**
    ...
    {
        if ($this->report === null) {
            $this->report = (new \SebastianBergmann\CodeCoverage\Node\Builder())->build($this);
        }
        return $this->report;
    }
    /**
    ...
    public function stop(bool $append = \true, $linesToBeCovered = [], array $linesToBeUsed = [], bool $ignoreForceCoversAnnotation = \false) : array
    {
        if (!\is_array($linesToBeCovered) && $linesToBeCovered !== \false) {
            throw \SebastianBergmann\CodeCoverage\InvalidArgumentException::create(2, 'array or false');
        }
    ...
    public function append(array $data, $id = null, bool $append = \true, $linesToBeCovered = [], array $linesToBeUsed = [], bool $ignoreForceCoversAnnotation = \false) : void
    {
        ...
        if ($id === null) {
            throw new \SebastianBergmann\CodeCoverage\RuntimeException();
        }
        ...
        foreach ($data as $file => $lines) {
            ...
            foreach ($lines as $k => $v) {
                if ($v === \SebastianBergmann\CodeCoverage\Driver\Driver::LINE_EXECUTED) {
    ...
    private function applyCoversAnnotationFilter(array &$data, $linesToBeCovered, array $linesToBeUsed, bool $ignoreForceCoversAnnotation) : void
    {
        if ($linesToBeCovered === \false || $this->forceCoversAnnotation && empty($linesToBeCovered) && !$ignoreForceCoversAnnotation) {
            if ($this->checkForMissingCoversAnnotation) {
                throw new \SebastianBergmann\CodeCoverage\MissingCoversAnnotationException();
            }
            $data = [];
            return;
        }
        ...
        if ($this->checkForUnintentionallyCoveredCode && (!$this->currentId instanceof \PHPUnit\Framework\TestCase || !$this->currentId->isMedium() && !$this->currentId->isLarge())) {
    ...
    private function addUncoveredFilesFromWhitelist() : void
    {
        ...
        foreach ($uncoveredFiles as $uncoveredFile) {
            ...
            for ($i = 1; $i <= $lines; $i++) {
                $data[$uncoveredFile][$i] = \SebastianBergmann\CodeCoverage\Driver\Driver::LINE_NOT_EXECUTED;
            }
        }
    ...
    private function getLinesToBeIgnoredInner(string $fileName) : array
    {
        ...
        if ($this->cacheTokens) {
            $tokens = \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_Stream_CachingFactory::get($fileName);
        } else {
            $tokens = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_Stream($fileName);
        }
        ...
        foreach ($tokens->tokens() as $token) {
            switch (\get_class($token)) {
                case \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_COMMENT::class:
                case \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_DOC_COMMENT::class:
                    ...
                    break;
                case \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_INTERFACE::class:
                case \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_TRAIT::class:
                case \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_CLASS::class:
                case \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_FUNCTION::class:
                ...
                case \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_NAMESPACE::class:
                    $this->ignoredLines[$fileName][] = $token->getEndLine();
                // Intentional fallthrough
                case \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_DECLARE::class:
                case \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_OPEN_TAG::class:
                case \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_CLOSE_TAG::class:
                case \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_USE::class:
                case \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\PHP_Token_USE_FUNCTION::class:
                    $this->ignoredLines[$fileName][] = $token->getLine();
                    break;
            }
            if ($ignore) {
    ...
    private function performUnintentionallyCoveredCodeCheck(array &$data, array $linesToBeCovered, array $linesToBeUsed) : void
    {
        ...
        if (!empty($unintentionallyCoveredUnits)) {
            throw new \SebastianBergmann\CodeCoverage\UnintentionallyCoveredCodeException($unintentionallyCoveredUnits);
        }
    }
    /**
     * @throws CoveredCodeNotExecutedException
    ...
    private function performUnexecutedCoveredCodeCheck(array &$data, array $linesToBeCovered, array $linesToBeUsed) : void
    {
        ...
        if (!empty($message)) {
            throw new \SebastianBergmann\CodeCoverage\CoveredCodeNotExecutedException($message);
        }
    }
     ...
     * @throws RuntimeException
     */
    private function selectDriver(\SebastianBergmann\CodeCoverage\Filter $filter) : \SebastianBergmann\CodeCoverage\Driver\Driver
    {
        $runtime = new \SebastianBergmann\Environment\Runtime();
        if ($runtime->hasPHPDBGCodeCoverage()) {
            return new \SebastianBergmann\CodeCoverage\Driver\PHPDBG();
        }
        if ($runtime->hasPCOV()) {
            return new \SebastianBergmann\CodeCoverage\Driver\PCOV();
        }
        if ($runtime->hasXdebug()) {
            return new \SebastianBergmann\CodeCoverage\Driver\Xdebug($filter);
        }
        throw new \SebastianBergmann\CodeCoverage\RuntimeException('No code coverage driver available');
    }
        ...
        if ($this->processUncoveredFilesFromWhitelist) {
            ...
            foreach ($this->driver->stop() as $file => $fileCoverage) {
                ...
                foreach (\array_keys($fileCoverage) as $key) {
                    if ($fileCoverage[$key] === \SebastianBergmann\CodeCoverage\Driver\Driver::LINE_EXECUTED) {
                        $fileCoverage[$key] = \SebastianBergmann\CodeCoverage\Driver\Driver::LINE_NOT_EXECUTED;
                    }
                }
