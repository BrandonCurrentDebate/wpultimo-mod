...
final class Builder
{
    public function build(\SebastianBergmann\CodeCoverage\CodeCoverage $coverage) : \SebastianBergmann\CodeCoverage\Node\Directory
    {
        ...
        $root = new \SebastianBergmann\CodeCoverage\Node\Directory($commonPath, null);
        $this->addItems($root, $this->buildDirectoryStructure($files), $coverage->getTests(), $coverage->getCacheTokens());
        return $root;
    }
    private function addItems(\SebastianBergmann\CodeCoverage\Node\Directory $root, array $items, array $tests, bool $cacheTokens) : void
    {
