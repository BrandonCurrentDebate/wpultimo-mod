...
abstract class TypeNodeAbstract
{
    ...
    protected function guardIsValidType()
    {
        if ($this->types == ['null' => 'null']) {
            throw new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Exception\Doubler\DoubleException('Type cannot be standalone null');
        }
        if ($this->types == ['false' => 'false']) {
            throw new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Exception\Doubler\DoubleException('Type cannot be standalone false');
        }
        if ($this->types == ['false' => 'false', 'null' => 'null']) {
            throw new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Exception\Doubler\DoubleException('Type cannot be nullable false');
        }
        if (\PHP_VERSION_ID >= 80000 && isset($this->types['mixed']) && \count($this->types) !== 1) {
            throw new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Exception\Doubler\DoubleException('mixed cannot be part of a union');
        }
    }
}
