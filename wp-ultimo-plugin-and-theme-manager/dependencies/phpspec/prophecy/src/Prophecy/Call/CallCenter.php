 ...
 */
namespace WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Call;
...
use WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Exception\Call\UnexpectedCallException;
...
class CallCenter
{
     ...
     */
    public function makeCall(\WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Prophecy\ObjectProphecy $prophecy, $methodName, array $arguments)
    {
        ...
        if ('__destruct' === \strtolower($methodName) || 0 == \count($prophecy->getMethodProphecies())) {
            $this->recordedCalls[] = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Call\Call($methodName, $arguments, null, null, $file, $line);
            return null;
        }
        ...
        if (!\count($matches)) {
            $this->unexpectedCalls->attach(new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Call\Call($methodName, $arguments, null, null, $file, $line), $prophecy);
            $this->recordedCalls[] = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Call\Call($methodName, $arguments, null, null, $file, $line);
            return null;
        }
        ...
        if ($methodProphecy->hasReturnVoid() && $returnValue !== null) {
            throw new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Exception\Prophecy\MethodProphecyException("The method \"{$methodName}\" has a void return type, but the promise returned a value", $methodProphecy);
        }
        $this->recordedCalls[] = $call = new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Call\Call($methodName, $arguments, $returnValue, $exception, $file, $line);
     ...
     * @return Call[]
     */
    public function findCalls($methodName, \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Argument\ArgumentsWildcard $wildcard)
    {
        $methodName = \strtolower($methodName);
        return \array_values(\array_filter($this->recordedCalls, function (\WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Call\Call $call) use($methodName, $wildcard) {
    ...
    private function createUnexpectedCallException(\WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Prophecy\ObjectProphecy $prophecy, $methodName, array $arguments)
    {
        ...
        return new \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Exception\Call\UnexpectedCallException(\sprintf("Unexpected method call on %s:\n" . "  - %s(\n" . "%s\n" . "    )\n" . "expected calls were:\n" . "%s", $classname, $methodName, $argstring, \implode("\n", $expected)), $prophecy, $methodName, $arguments);
    }
     ...
     * @return array
     */
    private function findMethodProphecies(\WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Prophecy\ObjectProphecy $prophecy, $methodName, array $arguments)
    {
        $matches = array();
