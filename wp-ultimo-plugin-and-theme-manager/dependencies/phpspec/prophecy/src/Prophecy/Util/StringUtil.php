 ...
 */
namespace WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Util;
...
class StringUtil
{
    ...
    public function stringify($value, $exportObject = \true)
    {
        ...
        if (\is_object($value)) {
            return $exportObject ? \WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Util\ExportUtil::export($value) : \sprintf('%s:%s', \get_class($value), \spl_object_hash($value));
        }
    ...
    {
        $self = $this;
        return \implode(\PHP_EOL, \array_map(function (\WP_Ultimo_Plugin_And_Theme_Manager\Dependencies\Prophecy\Call\Call $call) use($self) {
