...
class DateTimeComparator extends \SebastianBergmann\Comparator\ObjectComparator
{
    ...
    public function assertEquals($expected, $actual, $delta = 0.0, $canonicalize = \false, $ignoreCase = \false, array &$processed = [])
    {
        ...
        if ($actualClone < $expectedLower || $actualClone > $expectedUpper) {
            throw new \SebastianBergmann\Comparator\ComparisonFailure($expected, $actual, $this->dateTimeToString($expected), $this->dateTimeToString($actual), \false, 'Failed asserting that two DateTime objects are equal.');
        }
    }
    /**
