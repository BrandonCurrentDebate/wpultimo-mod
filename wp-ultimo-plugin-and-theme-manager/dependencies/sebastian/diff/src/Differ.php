...
final class Differ
{
    ...
    {
        if ($outputBuilder instanceof \SebastianBergmann\Diff\Output\DiffOutputBuilderInterface) {
            $this->outputBuilder = $outputBuilder;
        } elseif (null === $outputBuilder) {
            $this->outputBuilder = new \SebastianBergmann\Diff\Output\UnifiedDiffOutputBuilder();
            ...
            // @deprecated
            $this->outputBuilder = new \SebastianBergmann\Diff\Output\UnifiedDiffOutputBuilder($outputBuilder);
        } else {
            throw new \SebastianBergmann\Diff\InvalidArgumentException(\sprintf('Expected builder to be an instance of DiffOutputBuilderInterface, <null> or a string, got %s.', \is_object($outputBuilder) ? 'instance of "' . \get_class($outputBuilder) . '"' : \gettype($outputBuilder) . ' "' . $outputBuilder . '"'));
        }
    }
    /**
    ...
    public function diffToArray($from, $to, \SebastianBergmann\Diff\LongestCommonSubsequenceCalculator $lcs = null) : array
    {
        if (\is_string($from)) {
            ...
            throw new \SebastianBergmann\Diff\InvalidArgumentException('"from" must be an array or string.');
        }
        if (\is_string($to)) {
            $to = $this->splitStringByLines($to);
        } elseif (!\is_array($to)) {
            throw new \SebastianBergmann\Diff\InvalidArgumentException('"to" must be an array or string.');
        }
    ...
    private function selectLcsImplementation(array $from, array $to) : \SebastianBergmann\Diff\LongestCommonSubsequenceCalculator
    {
        ...
        if ($this->calculateEstimatedFootprint($from, $to) > $memoryLimit) {
            return new \SebastianBergmann\Diff\MemoryEfficientLongestCommonSubsequenceCalculator();
        }
        return new \SebastianBergmann\Diff\TimeEfficientLongestCommonSubsequenceCalculator();
    }
    /**
