<?php
/**
 * WP Ultimo: Plugin and Theme Manager helper methods for including and rendering files, assets, etc
 *
...
namespace WP_Ultimo_Plugin_And_Theme_Manager; // phpcs:ignore
...
class Helper {
	...
	public function init() {
		...
		// Overwrite
		add_filter('wp_ultimo_ptm_view_override', array($this, 'view_override'), 10, 3);
	...
	public function path($dir) {
		...
		return WP_ULTIMO_PLUGIN_AND_THEME_MANAGER_PLUGIN_DIR . $dir;
	...
	public function url($dir) {
		...
		return apply_filters('wp_ultimo_plugin_and_theme_manager_url', WP_ULTIMO_PLUGIN_AND_THEME_MANAGER_PLUGIN_URL . $dir);
	...
	public function render($view, $vars = false) {
		...
		if (preg_match('/(' . implode('|', $this->replaceable_views) . ')\w+/', $view)) {
			...
			$template = apply_filters('wp_ultimo_plugin_and_theme_manager_view_override', $template, $view);
	...
	public function view_override($original_path, $view) {
		...
		if (is_main_site()) {
			...
			$found = locate_template("wu-ptm/$view.php");

		} else {

			$found = $this->custom_locate_template("wu-ptm/$view.php");
	...
	public function get_option($option_name = 'settings', $default = array()) {
		...
		return apply_filters('wp_ultimo_plugin_and_theme_manager_get_option', $option_value, $option_name, $default);
