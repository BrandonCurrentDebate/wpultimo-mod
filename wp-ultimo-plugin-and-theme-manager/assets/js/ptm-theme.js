...
/* global _wpThemeSettings, _, Backbone, wu_ptm, query, tb_position */
/*jshint -W093 */
window.wp = window.wp || {};
...
(function($) {
  ...
  const l10n = themes.data.pluginl10n || themes.data.l10n;
  ...
  themes.view.Appearance = wp.Backbone.View.extend({
    ...
    search_category(event) {
      ...
      this.collection.doSearch(event.target.value, true);
    ...
    scroller() {
      ...
      const bottom = this.window.scrollTop() + self.window.height();

      //threshold = self.$el.offset().top + self.$el.outerHeight(false) - self.window.height();
      threshold = self.$el.offsetTop + self.$el.outerHeight(false) - self.window.height();
  ...
  themes.Collection = Backbone.Collection.extend({
    ...
    // @uses RegExp
    search(term, cat_search = false) {

      let haystack, name, description, author, tags;
      ...
      // Find results
      // _.filter and .test
      const results = this.filter(function(data) {
        ...
        const has_category = _.contains(tags, term);

        haystack = _.union([name, data.get('id'), description, author]);
        ...
        return match.test(haystack) || has_category;
    ...
    paginate(instance) {
      ...
      collection = _(collection.rest(9999 * instance));

      collection = _(collection.first(9999));
    ...
    query(request) {
      ...
      const isPaginated = _.has(request, 'page');
      ...
      if (! query && ! isPaginated) {
        ...
        query = this.apiCall(request).done(function(data) {
          ...
          if (data.themes) {
            ...
            queries.push({ themes: data.themes, request, total: count });
  ...
  themes.view.Theme = wp.Backbone.View.extend({
    ...
    addFocus() {
      ...
      const $themeToFocus = ($(':focus').hasClass('theme')) ? $(':focus') : $(':focus').parents('.theme');
    ...
    preview(event) {
      ...
      const preview = new themes.view.Preview({
        model: this.model,
      });
    ...
    setNavButtonsState() {
      ...
      const $themeInstaller = $('.theme-install-overlay'),
    ...
    installTheme(event) {
      ...
      $(document).on('wp-theme-install-success', function(response) {
    ...
    updateTheme(event) {
      ...
      $(document).on('wp-theme-update-success', function(response) {
  ...
  themes.view.Details = wp.Backbone.View.extend({
    ...
    containFocus($el) {
      ...
      $el.on('keydown.wp-themes', function(event) {
        ...
        const $firstFocusable = $el.find('.theme-header button:not(.disabled)').first(),
    ...
    deleteTheme(event) {
      ...
      $(document).one('wp-theme-delete-success', function(response) {
    ...
    screenshotCheck(el) {
      ...
      const screenshot = el.find('.screenshot img');
  ...
  themes.view.Preview = themes.view.Details.extend({
    ...
    render() {
      ...
      const currentPreviewDevice = this.$el.data('current-preview-device');
    ...
    tooglePreviewDeviceButtons(newDevice) {

      const $devices = $('.wp-full-overlay-footer .devices');
  ...
  themes.view.Themes = wp.Backbone.View.extend({
    ...
    render() {
      ...
      if (themes.data.themes.length === 1 && wu_ptm.type === 'themes') {
    ...
    renderThemes(page) {
      ...
      if (! themes.isInstall && themes.data.settings.canInstall) {
        ...
        this.$el.append(
          ...
          '<div class="theme add-new-theme"><a href="' + themes.data.settings.installURI + '"><div class="theme-screenshot"><span></span></div><h2 class="theme-name">' + l10n.addNew + '</h2></a></div>'
        );
    ...
    currentTheme() {
      ...
      const current = self.collection.findWhere({ active: true });
    ...
    next(args) {
      ...
      const model = self.collection.get(args[0]);

      // Find the next model within the collection
      const nextModel = self.collection.at(self.collection.indexOf(model) + 1);
    ...
    previous(args) {
      ...
      const previousModel = self.collection.at(self.collection.indexOf(model) - 1);
  ...
  const getRoutes = function() {
    ...
    routes[wu_ptm.type + '.php?' + wu_ptm.type.replace('s', '') + '=:slug'] = 'theme';

    routes[wu_ptm.type + '.php?search=:query'] = 'search';

    routes[wu_ptm.type + '.php?s=:query'] = 'search';

    routes[wu_ptm.type + '.php'] = 'themes';
  ...
  themes.Router = Backbone.Router.extend({
    ...
    baseUrl(url) {
      ...
      return wu_ptm.type + '.php' + url;

    },

    themePath: '?' + wu_ptm.type.replace('s', '') + '=',
    searchPath: '?search=',
    ...
    search(query) {
      ...
      const target = $('[data-category="' + query + '"]');
  ...
  themes.view.Installer = themes.view.Appearance.extend({
    ...
    applyFilters(event) {
      ...
      const request = { tag: tags };

      const tags = this.filtersChecked();
      ...
      const filteringBy = $('.filtered-by .tags');
    ...
    saveUsername(event) {
      ...
      // eslint-disable-next-line max-len
      const username = $('#wporg-username-input').val(),
        request = { browse: 'favorites', user: username },
        that = this;

      const nonce = $('#wporg-username-nonce').val();
    ...
    filtersChecked() {
      ...
      const items = $('.filter-group').find(':checkbox'),
        tags = [];
    ...
    uploader() {
      ...
      const uploadViewToggle = $('.upload-view-toggle'),
        $body = $(document.body);
