<?php
/**
 * Check if all the pre-requisites to run WP Ultimo Captcha are in place.
 *
...
class Requirements {
	...
	/**
	 * Minimum PHP version required to run WP Ultimo Captcha.
	 *
	 * @since 1.0.0
	 * @var string
	 */
	...
	/**
	 * Minimum WordPress version required to run WP Ultimo Captcha.
	 *
	 * @since 1.0.0
	 * @var string
	 */
	 ...
	 * @var string
	 */
	public static $wp_ultimo_version = '2.0.0-beta.3';
	...
	/**
	 * Check if the minimum pre-requisites to run WP Ultimo Captcha are present.
	 *
	 ...
	 * - Check if WP Ultimo Captcha is network active.
	 *
	 * @since 1.0.0
	 * @return boolean
	 */
	...
	public static function run_setup() {
		...
		return get_network_option(null, 'wp_ultimo_captcha_setup_finished', false);
	...
	public static function check_php_version() {
		...
		if (version_compare(phpversion(), self::$php_version, '<')) {
			...
			add_action('network_admin_notices', array('WP_Ultimo_Captcha\Requirements', 'notice_unsupported_php_version'));
	...
	public static function check_wp_version() {
		...
		if (version_compare($wp_version, self::$wp_version, '<')) {
			...
			add_action('network_admin_notices', array('WP_Ultimo_Captcha\Requirements', 'notice_unsupported_wp_version'));
	...
	public static function check_wp_ultimo_version() {
		...
		if (!function_exists('\WP_Ultimo')) {
			...
			add_action('network_admin_notices', array('WP_Ultimo_Captcha\Requirements', 'notice_unsupported_wp_ultimo_version'));
	...
	public static function is_multisite() {
		...
		if (!is_multisite()) {
			...
			add_action('admin_notices', array('WP_Ultimo_Captcha\Requirements', 'notice_not_multisite'));
	...
	public static function is_network_active() {
		...
		if (!is_plugin_active_for_network(WP_ULTIMO_CAPTCHA_PLUGIN_BASENAME) && !self::is_unit_test()) {

			add_action('admin_notices', array('WP_Ultimo_Captcha\Requirements', 'notice_not_network_active'));
	...
	public static function notice_unsupported_php_version() {
		...
		$message = sprintf(__('WP Ultimo Captcha requires at least PHP version %1$s to run. Your current PHP version is <strong>%2$s</strong>. Please, contact your hosting company support to upgrade your PHP version. If you want maximum performance consider upgrading your PHP to version 7.0 or later.', 'wp-ultimo-captcha'), self::$php_version, phpversion());
	...
	public static function notice_unsupported_wp_version() {
		...
		$message = sprintf(__('WP Ultimo Captcha requires at least WordPress version %1$s to run. Your current WordPress version is <strong>%2$s</strong>.', 'wp-ultimo-captcha'), self::$wp_version, $wp_version);
	...
	public static function notice_not_multisite() {
		...
		$message = __('WP Ultimo Captcha requires a multisite install to run properly. To know more about WordPress Networks, visit this link: <a href="https://wordpress.org/support/article/create-a-network/">Create a Network &rarr;</a>', 'wp-ultimo-captcha');
	...
	/**
	 * Adds a network admin notice about the WP Ultimo Captcha not being network-active
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public static function notice_not_network_active() {
		...
		$message = sprintf(__('WP Ultimo Captcha needs to be network active to run properly. You can "Network Activate" it <a href="%s">here</a>', 'wp-ultimo-captcha'), network_admin_url('plugins.php'));
	...
	public static function notice_unsupported_wp_ultimo_version() {
		...
		$message = sprintf(__('WP Ultimo Captcha requires WP Ultimo version %1$s or later to be installed and activated to run.', 'wp-ultimo-captcha'), self::$wp_ultimo_version);
		printf('<div class="notice notice-error"><p>%s</p></div>', $message);
